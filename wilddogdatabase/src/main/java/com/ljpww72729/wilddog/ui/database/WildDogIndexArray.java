...
public class WildDogIndexArray<T> extends CachingObservableSnapshotArray<T> implements ChangeEventListener {
    private static final String TAG = "FirebaseIndexArray";

    private SyncReference mDataRef;
    private Map<SyncReference, ValueEventListener> mRefs = new HashMap<>();

    private WildDogArray<String> mKeySnapshots;
    private List<DataSnapshot> mDataSnapshots = new ArrayList<>();

    /**
     * When keys are added in {@link WildDogArray}, we need to fetch the data async. This list
     * contains keys that exist in the backing {@link WildDogArray}, but their data hasn't been
     ...
     * backing {@link WildDogArray} is done updating. This will be true if the backing {@link
     * WildDogArray} is in the middle of an update, false otherwise.
     */
     ...
     */
    public WildDogIndexArray(Query keyQuery, SyncReference dataRef, Class<T> tClass) {
        super(tClass);
     ...
     * @see WildDogIndexArray#WildDogIndexArray(Query, SyncReference, Class)
     */
    public WildDogIndexArray(Query keyQuery, SyncReference dataRef, SnapshotParser<T> parser) {
        super(parser);
        init(keyQuery, dataRef);
    }

    private void init(Query keyQuery, SyncReference dataRef) {
        mDataRef = dataRef;
        mKeySnapshots = new WildDogArray<>(keyQuery, new SnapshotParser<String>() {
            @Override
    ...
    protected void onDestroy() {
        ...
        for (SyncReference ref : mRefs.keySet()) {
    ...
    protected void onKeyAdded(DataSnapshot data) {
        ...
        SyncReference ref = mDataRef.child(key);
    ...
    public boolean equals(Object obj) {
        ...
        WildDogIndexArray array = (WildDogIndexArray) obj;
